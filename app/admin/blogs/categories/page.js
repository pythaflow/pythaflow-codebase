...
export default function BlogCategoriesAdmin() {
  ...
  const [formData, setFormData] = useState({ name: '', slug: '', status: true });
  ...
  const handleEdit = (c) => {
    setFormData({ name: c.name, slug: c.slug, status: c.status });
    setEditingId(c.id);
    setView('form');
  };
  ...
  const resetForm = () => {
    setFormData({ name: '', slug: '', status: true });
    setEditingId(null);
    setView('list');
  };
  ...
  if (view === 'form') return (
    <div style={{ width: '100%', maxWidth: '800px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <h2 style={{ fontSize: '1.5rem' }}>{editingId ? 'Edit Category' : 'New Category'}</h2>
  ...
  return (
    <div style={{ width: '100%' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '2rem' }}>
        ...
        <button onClick={() => setView('form')} style={{ background: 'var(--accent)', color: '#fff', border: 'none', padding: '0.5rem 1rem', borderRadius: '4px', cursor: 'pointer' }}>+ Add Category</button>
      </div>
      {loading ? <p>Loading...</p> : (
          ...
          {categories.map(c => (
            <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', background: 'var(--bg)', padding: '1.5rem', border: '1px solid var(--border)', borderRadius: '8px' }}>
              <div>
                ...
                <p style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Slug: {c.slug} | {c.status ? 'Active' : 'Inactive'}</p>
              </div>
